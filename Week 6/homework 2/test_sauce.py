...
class Test_Sauce:
    ...
    @pytest.mark.parametrize("username,password",[("problem_user","secret_sauce"),("standard_user","secret_sauce")])
    def test_img_sources(self,username,password):
        self.wait_until((By.ID,"login-button"))
        usernameSection = self.driver.find_element(By.ID,"user-name")
        passwordSection = self.driver.find_element(By.ID,"password")
        loginBtn = self.driver.find_element(By.ID,"login-button")
        ...
        self.wait_until((By.CLASS_NAME,"inventory_item_img"))

        images = self.driver.find_elements(By.CLASS_NAME,"inventory_item_img")
        ...
        self.driver.save_screenshot(f"{self.folderPath}/test_img_sources{username,password}.png") 

        for i in range(len(imgSrcs)):
            for j in range(i+1, len(imgSrcs)):
                if imgSrcs[i] is not None and imgSrcs[j] is not None: 
                    assert imgSrcs[i] != imgSrcs[j]


    def test_empty_first_name_checkout(self):
        self.login()
        self.wait_until((By.CLASS_NAME,"btn_inventory"))
        buttonAddToCart = self.driver.find_element(By.CLASS_NAME,"btn_inventory")
        buttonAddToCart.click()

        self.wait_until((By.CLASS_NAME,"shopping_cart_link"))
        buttonShpCart = self.driver.find_element(By.CLASS_NAME,"shopping_cart_link")
        buttonShpCart.click()

        WebDriverWait(self.driver,5).until(ec.url_to_be(f"{self.url}cart.html"))

        self.wait_until((By.CLASS_NAME,"checkout_button"))
        buttonCheckOut = self.driver.find_element(By.CLASS_NAME,"checkout_button")

        buttonCheckOut.click()

        WebDriverWait(self.driver,5).until(ec.url_to_be(f"{self.url}checkout-step-one.html"))

        self.wait_until((By.ID, "continue"))

        buttonContinue = self.driver.find_element(By.ID, "continue")

        buttonContinue.click()

        self.wait_until((By.CSS_SELECTOR,"#checkout_info_container > div > form > div.checkout_info > div.error-message-container.error > h3"))
        error = self.driver.find_element(By.CSS_SELECTOR,"#checkout_info_container > div > form > div.checkout_info > div.error-message-container.error > h3")

        self.driver.save_screenshot(f"{self.folderPath}/test_empty_first_name_checkout.png") 
        assert error.text == "Error: First Name is required"

    def test_za_filter(self):
        self.login()
        self.wait_until((By.CLASS_NAME,"product_sort_container"))
        ...
        self.wait_until((By.CLASS_NAME,"inventory_item_name"))
        itemName = self.driver.find_element(By.CLASS_NAME,"inventory_item_name")

        self.driver.save_screenshot(f"{self.folderPath}/test_za_filter.png") 

        assert itemName.text == "Test.allTheThings() T-Shirt (Red)"
    ...
    def test_lohi_filter(self):
        ...
        self.driver.save_screenshot(f"{self.folderPath}/test_lohi_filter.png") 
        assert itemName.text == "Sauce Labs Onesie"
    ...
    def test_hilo_filter(self):
        ...
        self.driver.save_screenshot(f"{self.folderPath}/test_hilo_filter.png") 
        assert itemName.text == "Sauce Labs Fleece Jacket"

    def test_logout(self):
        self.login()

        self.wait_until((By.ID,"react-burger-menu-btn"))
        menu = self.driver.find_element(By.ID,"react-burger-menu-btn")

        menu.click()
        self.wait_until((By.ID,"logout_sidebar_link"))
        logoutBtn = self.driver.find_element(By.ID,"logout_sidebar_link")

        logoutBtn.click()
        WebDriverWait(self.driver,5).until(ec.url_to_be(self.url))
        self.driver.save_screenshot(f"{self.folderPath}/test_logout.png") 
        assert ec.url_to_be(self.url)